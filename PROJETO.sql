...
CURSOR CRS
IS
  SELECT MODELO.NOME AS Modelo ,MONTADORA.NOME AS Montadora,Count(*) AS Total
...
BEGIN
  ...
  EXIT WHEN CRS%NOTFOUND;
    Dbms_Output.Put_Line(total_clientes || ' cliente(s) possuem veiculos do modelo '||modelo || '- '||montadora);
  END LOOP;
  CLOSE CRS;
END;
...
CREATE OR REPLACE PROCEDURE ultimas_apolices AS
  CURSOR C1 IS
    SELECT TO_CHAR(A.DATACADASTRO,'DD/MM/YYYY') data,
           C.NOME nome,
           MODELO.NOME Modelo,
           Concat('R$ ',to_char(A.VALORAPOLICE, 'FM999G999G999D90')) Valor
           FROM JMM_APOLICE A
...
EXECUTE PKG_CRUD.ADD_VEICULO('JXA-1222',1990,5,2);
EXECUTE PKG_CRUD.EDITAR_VEICULO(6,'JSA-1222','JJQ-1221');
EXECUTE PKG_CRUD.EXCLUIR_VEICULO(5); --VERIFICAR AS REGRAS DE FK
...
BEGIN
  IF(To_Char(SYSDATE,'D')) IN (7,8) THEN
    IF(INSERTING) THEN
